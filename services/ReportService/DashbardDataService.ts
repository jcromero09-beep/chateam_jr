  ...
  const query = `   with traking as
                         ...
                         counters
                           as (select (select avg("supportTime") from traking where "supportTime" > 0) "avgSupportTime",
                                      ...
                                      (select count(id) from traking where "status" = 'closed' and traking."rated" = false)           "waitRating",
                                      (select count(tt.id)
                                       from traking tt
                                              inner join "UserRatings" ur on ur."ticketId" = tt."ticketId"
                                       where tt."status" = 'closed' and (tt."rated" = false and ur."rate" = 0))                  "withoutRating",
                                      (select count(id)
                                       from traking
                                       where traking."rated" = true)                   "withRating",
                                      (((select count(id) from traking where traking."rated" = true) * 100) /
                                       nullif((select count(id) from traking), 0))                     "percRating",
                                      (select (100 * count(tt.*)) / NULLIF((select count(*) total
                                                                            from traking tt
                                                                                   inner join "UserRatings" ur on ur."ticketId" = tt."ticketId"
                                                                            where tt."rated" = true), 0)
                                       from traking tt
                                              inner join "UserRatings" ur on ur."ticketId" = tt."ticketId"
                                       where tt."rated" = true
                                      ...
                                      (select sum(nps.promoter) - sum(nps.detractor)
                                       from ( (select (100 * count(tt.*)) / NULLIF((select count(*) total
                                                                                    from traking tt
                                                                                           inner join "UserRatings" ur on ur."ticketId" = tt."ticketId"
                                                                                    where tt."rated" = true), 0) promoter,
                                                      0                                                     detractor
                                               from traking tt
                                                      inner join "UserRatings" ur on ur."ticketId" = tt."ticketId"
                                               where tt."rated" = true
