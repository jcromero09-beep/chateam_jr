...
export const ActionsWebhookService = async (
  ...
  try {
    ...
    console.log("🛠️ [ActionsWebhookService] Ejecutando flujo");
    console.log("➡️ idFlowDb:", idFlowDb);
    console.log("➡️ next:", nextStage);
    console.log("➡️ nodes:", JSON.stringify(nodes, null, 2));
    console.log("➡️ connects:", JSON.stringify(connects, null, 2));
    ...
    for (var i = 0; i < lengthLoop; i++) {
      ...
      if (pressKey) {
        ...
        if (execFn === "") {
          console.log("UPDATE5...");
          nodeSelected = nodes.find(n => n.id === nextStage && n.type === "menu");
        } else {
          console.log("UPDATE6...");
      ...
      //         interactiveMessage: {
      //           body: {
      //             text: nodeSelected.data.title || 'Botón URL',
      //           },
      //           footer: {
      //             text: nodeSelected.data.description || '',
      //           },
      //           nativeFlowMessage: {
      //             buttons: [
      //               {
      ...
      //                   display_text: nodeSelected.data.buttonText || 'Ir al sitio',
      ...
      //   await ticketDetails.update({
      //     lastMessage: nodeSelected.data.title
      //   });
      //   await intervalWhats("1");
      // }
      ...
      if (nodeSelected.type === "url") {
        ...
        const msgTemplate = {
          message: {
            templateMessage: {
              hydratedTemplate: {
                templateId: nodeSelected.data.title, // nombre de tu plantilla registrada
                hydratedContentText: ticketDetails.contact.name,
                hydratedButtons: [
                  {
                    index: 0,
                    urlButton: {
                      displayText: nodeSelected.data.buttonText || "Ir al sitio",
                      url: nodeSelected.data.url
                    }
                  }
                ]
        ...
        const newMsg = generateWAMessageFromContent(number, msgTemplate.message, {
          userJid: whatsapp.number
        });
              ...
              mediaPath:
                process.env.BACKEND_URL === "https://localhost:8090"
                  ? `${__dirname
                      ...
                      .split("\\")
                      .join("/")}public/${
                      nodeSelected.data.elements.find(
                        item => item.number === elementNowSelected
                      )?.value
                    }`
                  : `${__dirname
      ...
      //     // Guardar versión original del input recibido
      //     const originalPressKey = pressKey;
        
      //     // Intentar mapear texto a número
      //     const matchedOption = options.find(opt => {
      //       const cleanInput = pressKey.trim().toLowerCase();
      //       const cleanValue = opt.value.trim().toLowerCase();
      //       return cleanInput === cleanValue;
      //     });
      ...
      //     // ✅ Marcar que se procesó esta respuesta
      //     pressKey = "999";
        
      //     // Actualiza ticket con el valor seleccionado para que flowBuilderQueue funcione
      //     if (ticket && matchedOption) {
      //       await ticket.update({
      //         lastMessage: pressKey, // o matchedOption.number
      //         status: "pending" // obligatorio para que pase el filtro en flowBuilderQueue
      //       });
      //     }
        
      //     const isNodeExist = nodes.find(item => item.id === execFn);
      //     isMenu = isNodeExist?.type === "menu" || false;
      //   }
      ...
      //     // });
      //     const number = `${numberClient}@s.whatsapp.net`; // o .g.us si es grupo
      //     const wbot = getWbot(whatsapp.id);
      ...
      //           title: item.value,
      //           rowId: `menu_${nodeSelected.id}_${item.number}`, // este ID lo usarás en el listener de respuesta
      //           description: ""
      //         }))
      //       }
      //     ];
      ...
      //     const listMessage = {
      //       text: nodeSelected.data.message || "Selecciona una opción:",
      //       title: nodeSelected.data.title ||"Menú de opciones",
      //       buttonText: "Ver opciones",
      //       footer: nodeSelected.data.description|| "Chateam",
      //       sections
      //     };

      //     // try {
      //     //   await wbot.sendMessage(number, { listMessage });
      //     //   console.log("✅ Lista enviada");
      //     // } catch (err) {
      //     //   console.error("❌ Error al enviar lista:", err);
      //     // }
      //     try {
      //       await wbot.sendMessage(number, {
      //         text: nodeSelected.data.message || "Selecciona una opción:",
      //         title: nodeSelected.data.title ||"Menú de opciones",
      //         buttonText: "Ver opciones",
      //         footer: nodeSelected.data.description|| "Chateam",
      ...
      //             rows: nodeSelected.data.arrayOption.map(item => ({
      //               title: item.value,
      //               rowId: `menu_${nodeSelected.id}_${item.number}`,
      //               description: ""
      //             }))
      //           }
      //         ]
      ...
      //  const menuCreate = 
      //    `${nodeSelected.data.title || "Menú de opciones"}\n` +
      //    `${nodeSelected.data.message || ""}\n\n` +
      //    `${optionsText}\n\n` +
      //    `${nodeSelected.data.description || ""}`;
      ...
      console.log("273", nodeSelected.type);
