...
export default async function ListTicketsServiceReport(
  ...
  const searchOnMessages = params.searchOnMessages === "true" ? true : false;
  let query = "";
  console.log(params)
  if (onlyRated) {
   query = `
  ...
  if (params.searchParam && params.searchParam.trim() !== "") {
    const sanitizedSearchParam = params.searchParam.toLowerCase().trim();
    if (searchOnMessages) {
      query += ` and (
        LOWER(c."name") LIKE '%${sanitizedSearchParam}%' OR
        LOWER(t."lastMessage") LIKE '%${sanitizedSearchParam}%' OR
        EXISTS (
          SELECT 1 FROM "Messages" m
          WHERE m."ticketId" = t.id
          AND LOWER(m.body) LIKE '%${sanitizedSearchParam}%'
        )
      )`;
    } else {
      query += ` and (
        LOWER(c."name") LIKE '%${sanitizedSearchParam}%' OR
        LOWER(t."lastMessage") LIKE '%${sanitizedSearchParam}%'
      )`;
    }
  }
