> ## ⚠️ HISTÓRICO (Feb-2026) — PARCIALMENTE SUPERADO
> **El 100% de sus checkboxes está `[ ]`, pero buena parte YA se construyó** bajo otro nombre
> en `PLAN-FASE-2.md` (Jul-2026): A/B testing (`ABTestService` z-test), ROAS real (`RoasService`),
> atribución normalizada, motor de reglas (`AutomationRule`), scoring de creativos
> (`CreativeScoringService`), copy IA (`AdCopyGeneratorService`), detección de anomalías
> (`AnomalyDetectionService`), CTWA e2e (`CtwaCampaignService`). Genuinamente pendiente aún:
> CRUD genérico de campañas no-CTWA vía UI, explorador de intereses, audiencias custom/lookalike,
> WhatsApp Flows, segmentación por temperatura, reportes programados. Plan vigente:
> **`docs/PLAN_INTEGRAL_OLAS_2026_07.md`** (Ola 7).
