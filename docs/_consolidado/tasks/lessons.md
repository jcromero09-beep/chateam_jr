...
## Sesion 2026-05-28 — Kanban Lead Conversions (Meta CAPI)

| # | Leccion | Contexto |
|---|---------|----------|
| 38 | El servicio `SendWebsiteEvent.ts` ya resuelve destino Meta CAPI (Pixel/Dataset + token) por candidate companyIds (env → context → BD). REUSAR esa resolución exportando `resolveConversionDestination` + `buildUserData` + `getApiVersion` en lugar de duplicar lógica. Add "Lead" al tipo `WebsiteEventName`. NO tocar el flujo Purchase. | services/FacebookConversionService/SendWebsiteEvent.ts |
| 39 | Para enviar conversiones desde asignación de etiqueta Kanban: usar wrapper `setImmediate` (NON-BLOCKING) y `try/catch` en el caller. La asignación de etiqueta NUNCA debe romperse si Meta falla. | services/FacebookConversionService/KanbanLeadConversionService.ts |
| 40 | Tabla nueva `KanbanLeadConversionEvents` con UNIQUE INDEX `(companyId, contactId, kanbanKey, eventName)` para deduplicar por contacto+etapa+evento. Sequelize sync crea la tabla; los índices únicos compuestos los crea una migración SQL idempotente con `showIndex` + `addIndex`. | database/migrations/20260528100001-create-kanban-lead-conversion-events.ts |
| 41 | `eventId` estable formato `kanban_lead_${companyId}_${contactId}_${kanbanKey}` — permite a Meta deduplicar nativamente además de nuestra deduplicación en BD. `kanbanKey` se sanitiza para chars seguros. | KanbanLeadConversionService.buildEventId |
| 42 | Integración limpia con TicketTagController.store (asignación manual) y stageClassifier.worker (asignación por IA): solo agregar `sendKanbanLeadConversionFromTagAssignmentAsync({...})` después de `handleTagAssignment` cuando `tag.kanban === 1`. | controllers/TicketTagController.ts, workers/stageClassifier.worker.ts |
| 43 | El endpoint `GET /ticket-notes/list` daba 500: el modelo `TicketNote` declara `contactId` + `BelongsTo(Contact)` pero la tabla real `TicketNotes` NO tiene esa columna (solo id, note, ticketId, userId, createdAt, updatedAt). Fix: filtrar solo por `ticketId`, acotar `attributes` a columnas reales y quitar el include de Contact. Verificar siempre modelo vs `\d tabla` antes de consumir un endpoint nuevo. | services/TicketNoteService/FindNotesByContactIdAndTicketId.ts |
